...
if( $o{help} ) {
  print << "END";
$0 [--sqft=<min sq ft>] [--pricemin=dollars] [--pricemax=dollars] [--regex=search] [--newer-than=minutes] [--format=<html|plain>]
...
my $debug = 0;
my ($sqft,$pricemin,$pricemax,$regex,$newerthan) = @o{'sqft','pricemin','pricemax','regex','newer-than'};

if( defined $o{debug} && $o{debug} ) { $debug = 1; }

# Add Craigslist RSS search feeds here:
my @feeds = ( 
    'http://seattle.craigslist.org/search/apa/see?query=house&srchType=A&format=rss',
);
...
if( $format eq 'html' ) {
  $message .= $style;
  $message .= "<h1>Craigslist apt search!</h1>";
...
for my $feed (@feeds)
{
    ...
    for my $item (map { $Item{$_}; } sort { $Item{$a}->{age} <=> $Item{$b}->{age} } keys %Item) {
        ...
        if( defined $sqft && $sqft > 1 ) {
          debug "Doing sqft: $sqft";

          next if ! $s; # Show listings without square footage
          if( $s && $s < $sqft ) {
            debug "$s < $sqft";
            $showthisone = 0;
          }
        ...
        if( $showthisone ) {
          ...
          if( $format eq 'html' ) {
            $message .= qq|<tr><td><a href="$url">click</a></td><td>$title</td><td>\$$amount</td><td>sqft: $s</td><td>Age: $age_minutes minutes</tr>\n|;
          }
        }
    }
